...
import com.hyozu.example.ems_patient.Model.TripPlanModel;
import com.hyozu.example.ems_patient.Remote.IGoogleAPI;
import com.hyozu.example.ems_patient.Remote.RetrofitClient;
...
public class RequestDriverActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    @BindView(R.id.finding_your_ambulance)
    CardView finding_your_ambulance;
    @BindView(R.id.confirm_driver_layout)
    CardView confirm_driver_layout;
    @BindView(R.id.btn_confirm_driver)
    Button btn_confirm_driver;
    @BindView(R.id.confirm_pickup_driver_layout)
    ...
    @OnClick(R.id.btn_confirm_driver)
    void onConfirmdriver() {
        confirm_pickup_layout.setVisibility(View.VISIBLE); //show pickup
        confirm_driver_layout.setVisibility(View.GONE); //hide driver layout
    ...
    private void addMarkerWithPulseAnimation() {
        ...
        finding_your_ambulance.setVisibility(View.VISIBLE);
    ...
    private void findNearbyDriver(SelectePlaceEvent selectePlaceEvent) {
        if (Common.driversFound.size() > 0)
        {
            ...
            DriverGeoModel foundDriver = null; //Common.driversFound.get(Common.driversFound.keySet().iterator().next());
            ...
            for (String key:Common.driversFound.keySet())
            {
              ...
              //  Snackbar.make(main_layout, new StringBuilder("Driver Found: ")
            ...
            {
                Toast.makeText(this, getString(R.string.no_driver_accepts), Toast.LENGTH_SHORT).show();
                lastDriverCall = null;
                finish();
            }
        }
    ...
    private void setDataPickup() {
        txt_pickup.setText(txt_origin != null ? txt_origin.getText() : "None");
        mMap.clear();
        addPickMarker();
    }
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (dataSnapshot.exists())
                        {
                            ...
                            compositeDisposable.add(iGoogleAPI.getDirections("driving", "less_driving",
                                    tripPlanModel.getOrigin(),driverLocation,getString(R.string.google_api_key))
                                    ...
                                    .subscribe(returnResult ->{
                                        ...
                                        try {
                                            ...
                                            Glide.with(RequestDriverActivity.this)
                                                    ...
                                                    .into(img_driver);
                                            driver_name_txt.setText(tripPlanModel.getDriverInfoModel().getFirstName());

                                            confirm_pickup_layout.setVisibility(View.GONE);
                                            confirm_driver_layout.setVisibility(View.GONE);
                            ...
                            Snackbar.make(main_layout,getString(R.string.trip_not_found)+event.getTripId(),Snackbar.LENGTH_SHORT).show();
                    }
                ...
                .subscribe(returnResult ->{
                    ...
                    try {
                       ...
                       handler = new Handler(Looper.getMainLooper());
                        ...
                        driverOldPosition = to;// new driver position
    ...
    @Subscribe(sticky = true,threadMode = ThreadMode.MAIN)
    public void DeclineRequestAndRemoveTripFromDriver(DeclineRequestAndRemoveTripFromDriver event)
    {
        if (lastDriverCall != null)
        {
            Common.driversFound.get(lastDriverCall.getKey()).setDecline(true);
    ...
    private void drawPath(SelectePlaceEvent selectePlaceEvent) {
        compositeDisposable.add(iGoogleAPI.getDirections("driving", "less_driving",
                selectePlaceEvent.getOriginString(),selectePlaceEvent.getDestinationString(),getString(R.string.google_api_key))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(returnResult ->{
                    Log.d("API_RETURN",returnResult);
                    ...
                    try {
                       ...
                       blackPolyline.setPoints(p);
                        ...
                        JSONObject time = legObjects.getJSONObject("duration");
                        String duration = time.getString("text");
                        int durationValue = time.getInt("value");

                        JSONObject distanceJSON = legObjects.getJSONObject("distance");
                        String distance = distanceJSON.getString("text");
                        int distanceValue = distanceJSON.getInt("value");
                        ...
                        JSONObject startLocation = legObjects.getJSONObject("start_location");
                        JSONObject endLocation = legObjects.getJSONObject("end_location");

                        //update
                        selectePlaceEvent.setOriginAddress(start_address);
                        selectePlaceEvent.setOrigin(new LatLng(startLocation.getDouble("lat"), startLocation.getDouble("lng")));

                        selectePlaceEvent.setDestinationAddress(end_address);
                        selectePlaceEvent.setDestination(new LatLng(endLocation.getDouble("lat"), endLocation.getDouble("lng")));

                        selectePlaceEvent.setDurationValue(durationValue);
                        selectePlaceEvent.setDistanceValue(distanceValue);

                        //Calculate Fare
                        double fare = Common.calculateFareBasedOnMetter(distanceValue);
                        selectePlaceEvent.setTotalFare(fare);
                        txt_fare.setText(new StringBuilder("RS.").append(fare));
